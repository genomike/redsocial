...
namespace redsocial.test.application
{
    ...
    {
        [Fact]
        public async Task CreateEmpleado_WithValidData_ShouldReturnNewEmpleado()
        {
            ...
            var empleado = await empleadoService.CreateEmpleado(name, email);
        ...
        [Fact]
        public async Task CreateEmpleado_WithDuplicateEmail_ShouldThrowException()
        {
            // GIVEN: Un servicio de empleados con un empleado existente
            var empleadoService = new EmpleadoService();
            var email = "duplicate@company.com";
            await empleadoService.CreateEmpleado("First User", email);

            // WHEN: Se intenta crear otro empleado con el mismo email
            // THEN: Se debe lanzar una excepción
            await Assert.ThrowsAsync<InvalidOperationException>(async () => 
                await empleadoService.CreateEmpleado("Second User", email));
        }

        [Fact]
        public async Task GetEmpleadoById_WithValidId_ShouldReturnCorrectEmpleado()
        {
            // GIVEN: Un servicio de empleados con un empleado existente
            var empleadoService = new EmpleadoService();
            var expectedName = "Test Employee";
            var empleado = await empleadoService.CreateEmpleado(expectedName, "test@company.com");

            // WHEN: Se busca el empleado por su ID
            var foundEmpleado = await empleadoService.GetEmpleadoById(empleado.Id);
        ...
        [Fact]
        public async Task GetEmpleadoById_WithInvalidId_ShouldReturnNull()
        {
            ...
            var foundEmpleado = await empleadoService.GetEmpleadoById(invalidId);
